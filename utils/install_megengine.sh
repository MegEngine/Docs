...
pushd ${CUR_DIR}/../../ >/dev/null
    ...
    MGE_VER=$(aws --endpoint-url=http://oss-internal.hh-b.brainpp.cn s3 ls s3://megbrain-integraion-test/wheels/megengine/ | awk '{split($0, a, "-"); print a[4]}' | sed 's/[a-z+].*//' | sed 's/\.$//' | uniq | sort -V | tail -n 1)

    # install MegEngine, kai has configured for oss, kai has python3.8
    echo "installing newest version($MGE_VER) of megengine..."
